...
export class ApiWoocommerceService {
    ...
    async syncProductSingle(companyId: string, createProductDto: CreateProductDto, productId: Types.ObjectId) {
        const wooCategoriesId = await this.categoryMappingService.syncCategoryMappingsWithWooCommerce(companyId, createProductDto.id_category.toString(), createProductDto.id_sub_category.toString());
